...
Ça copie quickref dans `%LOCALAPPDATA%\quickref` et crée un raccourci avec la touche globale **`Ctrl+Alt+W`**.

> Note : la touche globale par défaut est `Ctrl+Alt+W`, choisie pour être déclenchable d'une seule main gauche sur AZERTY (W est en bas à gauche, près de Ctrl/Alt). Windows impose au minimum `Ctrl+Alt+lettre` pour un raccourci `.lnk`. Tu peux la changer en éditant `$Hotkey` en haut de `install.ps1`, puis relancer l'installation.
